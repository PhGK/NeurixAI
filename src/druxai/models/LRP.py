...
def reverse_feature_expansion_interaction(frame):
    ...
    frame_unexpanded = (
        LRP_frame.groupby(["therapy", "diagnostics", "sample_name"])["LRP"]
        .sum()
        .reset_index()
    )
...
def generate_inputs(therapy_of_one_sample, diagnostics_of_one_sample):
    assert (
        therapy_of_one_sample.name == diagnostics_of_one_sample.name
    ), "something is off"

    th = pd.DataFrame(
        {"therapy_input": therapy_of_one_sample, "therapy": therapy_of_one_sample.index}
    )

    dia = pd.DataFrame(
        {
            "diagnostics_input": diagnostics_of_one_sample,
...
def calculate_LRP_interaction(
    model, ds, output_feature=0, PATH="./results/LRP/", fold=None
):
    ...
    for drug, molecular, outcome, idx in tqdm(dl):
        ...
        LRP_scores_long = np2pd(
            input_relevance,
            current_cell=ds.cases_now.iloc[idx.cpu().numpy(), :]["cell_line"],
            current_drug=ds.cases_now.iloc[idx.cpu().numpy(), :]["DRUG"],
            molecular_names=ds.molecular_names,
        )

        LRP_scores_long = LRP_scores_long.reset_index(drop=True)

        molecular_input = pd.DataFrame(
            molecular.cpu().numpy(), columns=ds.molecular_names
        )
        molecular_input["cell_line"] = np.array(
            ds.cases_now.iloc[idx.cpu().numpy(), :]["cell_line"]
        )
        molecular_input["DRUG"] = np.array(
            ds.cases_now.iloc[idx.cpu().numpy(), :]["DRUG"]
        )
...
def calculate_one_side_LRP(
    ...
    for drug, molecular, outcome, idx in tqdm(dl):
        ...
        helper_network.linear.weight = nn.Parameter(
            drug_latent.detach().view(s[0] * s[1]) * tc.eye(s[0] * s[1], device=device),
            requires_grad=True,
        )
        ...
        pred = helper_network.forward(
            molecular_latent.view(s[0] * s[1])
        )  # .view(s[0],s[1])
        ...
        LRP_scores_long = np2pd(
            input_relevance,
            current_cell=ds.current_cases.iloc[idx.cpu().numpy(), :]["cell_line"],
            current_drug=ds.current_cases.iloc[idx.cpu().numpy(), :]["DRUG"],
            molecular_names=ds.molecular_names,
        )
        ...
        molecular_input["DRUG"] = np.array(
            ds.current_cases.iloc[idx.cpu().numpy(), :]["DRUG"]
        )
...
def np2pd_specific_genes(
    nparray, current_cell, current_drug, molecular_names, specific_genes
):
    tensor = nparray
...
def calculate_one_side_LRP_specific_genes(
    ...
    for drug, molecular, outcome, idx in tqdm(dl):
        ...
        LRP_scores_long = np2pd_specific_genes(
            input_relevance,
            current_cell=ds.current_cases.iloc[idx.cpu().numpy(), :]["cell_line"],
            current_drug=ds.current_cases.iloc[idx.cpu().numpy(), :]["DRUG"],
            molecular_names=ds.molecular_names,
    ...
    pd.concat(all_results, axis=0).to_csv(
        PATH + "LRP_specific_genes" + str(fold) + ".csv"
    )
