...
from druxai.models.NN import LRP_Linear
...
def reverse_feature_expansion_interaction(frame):
    ...
    frame_unexpanded = LRP_frame.groupby(['therapy', 'diagnostics', 'sample_name'])['LRP'].sum().reset_index()
...
def np2pd(nparray, current_cell, current_drug, molecular_names):
    ...
    long_frame = pd.melt(frame, id_vars = ['DRUG', 'cell_line'], var_name = 'molecular_names', value_name = 'LRP')
...
def generate_inputs(therapy_of_one_sample, diagnostics_of_one_sample):
    ...
    th = pd.DataFrame({'therapy_input': therapy_of_one_sample, 'therapy': therapy_of_one_sample.index})

    dia = pd.DataFrame({'diagnostics_input': diagnostics_of_one_sample, 'diagnostics': diagnostics_of_one_sample.index})
...
def calculate_LRP_interaction(model, ds, output_feature = 0, PATH = './results/LRP/', fold=None):
    ...
    for drug, molecular, outcome, idx in tqdm(dl):
        ...
        LRP_scores_long = np2pd(input_relevance, current_cell = ds.cases_now.iloc[idx.cpu().numpy(),:]['cell_line'],current_drug = ds.cases_now.iloc[idx.cpu().numpy(),:]['DRUG'], molecular_names = ds.molecular_names)
        ...
        molecular_input['cell_line'] = np.array(ds.cases_now.iloc[idx.cpu().numpy(),:]['cell_line'])
        molecular_input['DRUG'] = np.array(ds.cases_now.iloc[idx.cpu().numpy(),:]['DRUG'])


        molecular_input_long = pd.melt(molecular_input, id_vars = ['DRUG', 'cell_line'], var_name = 'molecular_names', value_name = 'expression')

        result_long = LRP_scores_long.merge(molecular_input_long, how='left')

        result_long.to_csv(PATH + 'LRP_' + str(fold) + '.csv',mode='w' if it==0 else 'a', header=it==0)
        it +=1
...
def calculate_one_side_LRP(model, ds, output_feature = 0, PATH = './results/LRP/', fold=None):
    ...
    for drug, molecular, outcome, idx in tqdm(dl):
        ...
        helper_network.linear.weight = nn.Parameter(drug_latent.detach().view(s[0] * s[1])*tc.eye(s[0]* s[1], device = device), requires_grad=True)
        helper_network.linear.bias = nn.Parameter(tc.zeros_like(helper_network.linear.bias), requires_grad=True)
        ...
        LRP_scores_long = np2pd(input_relevance, current_cell = ds.current_cases.iloc[idx.cpu().numpy(),:]['cell_line'],current_drug = ds.current_cases.iloc[idx.cpu().numpy(),:]['DRUG'], molecular_names = ds.molecular_names)
        ...
        molecular_input = pd.DataFrame(molecular.cpu().numpy(), columns = ds.molecular_names)
        molecular_input['cell_line'] = np.array(ds.current_cases.iloc[idx.cpu().numpy(),:]['cell_line'])
...
def calculate_one_side_LRP_specific_genes(model, ds, specific_genes,  output_feature = 0, PATH = './results/LRP_specific_genes/', fold=None):
    if not os.path.exists(PATH):
        os.makedirs(PATH)
    device = tc.device('cuda:0')
    #ds.generate_train_or_test_cases('test')
    ...
    for drug, molecular, outcome, idx in tqdm(dl):
        ...
        LRP_scores_long = np2pd_specific_genes(input_relevance, current_cell = ds.current_cases.iloc[idx.cpu().numpy(),:]['cell_line'],current_drug = ds.current_cases.iloc[idx.cpu().numpy(),:]['DRUG'], molecular_names = ds.molecular_names, specific_genes = specific_genes)
