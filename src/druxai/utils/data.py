...
class MyDataSet(Dataset):
    def __init__(self, file_path, n_splits, results_dir):
        ...
        self.cases = pd.read_csv(
            os.path.join(
                file_path,
                'auc_secondary_screen_prediction_targets.csv'),
            index_col=0).reset_index(
            drop=True)
        self.molecular_data = pd.read_csv(os.path.join(file_path, 'rna_df.csv'), index_col=0)
        self.unique_drugs, unique_cell_lines = self.cases['DRUG'].drop_duplicates(
    ...
    def __getitem__(self, idx):
        ...
        current_drug_onehot = self.drug_onehot_tensor[:,
                                                      self.unique_drugs_array == current_drugname].squeeze()
        current_molecular_data = self.current_molecular_data_tensor[tc.tensor(
            self.current_molecular_data.index == current_model_id).squeeze(), :].squeeze()
    ...
    def change_fold(self, fold, train_test):
        ...
        self.cell_ids_test, self.cell_lines_test = self.cell_ids_test_lists[
            fold], self.cell_lines_test_lists[fold]

        self.cell_ids_train = np.setdiff1d(
            np.arange(
                self.molecular_data.shape[0]),
            self.cell_ids_test)
        ...
        self.current_molecular_data = self.molecular_data.loc[self.cell_lines_train_filtered,
                                                              :] if train_test == 'train' else self.molecular_data.loc[self.cell_lines_test_filtered, :]
        ...
        self.current_molecular_data_tensor = tc.tensor(
            np.array(self.current_molecular_data)).float()
        self.current_cases = self.cases_train.reset_index(
            drop=True) if train_test == 'train' else self.cases_test.reset_index(
            drop=True)
    ...
    def get_drug_vector(self, current_drugname):
        current_drug_embedding = self.drug_embeddings_tensor[:,
                                                             current_drugname == self.drug_embeddings.columns].squeeze()
        current_drug_fingerprint = self.drug_fingerprints_tensor[:, self.drug_fingerprints.columns == current_drugname].squeeze(
        ) if current_drugname in self.drug_fingerprints.columns else tc.zeros(self.drug_fingerprints_tensor.shape[1]).squeeze()

        current_drug_onehot = self.drug_onehot_tensor[:,
                                                      self.unique_drugs_array == current_drugname].squeeze()
        current_drug_embedding_with_dose = tc.cat(
            (current_drug_embedding, current_drug_fingerprint, current_drug_onehot), axis=0)
...
class LRPSet(MyDataSet):
    def change_fold(self, fold, train_test):
        ...
        self.current_cases = self.current_cases[np.isin(
            self.current_cases['DRUG'], self.chosen_drugs)].reset_index(drop=True)

        print(self.current_cases)
        print('a')
        self.current_cases = self.current_cases[['DRUG', 'cell_line', 'auc_per_drug']].groupby(
            ['DRUG', 'cell_line'])['auc_per_drug'].mean().reset_index()
        print(self.current_cases)
...
class LRPSet_CellLines(MyDataSet):
    def change_fold(self, fold, train_test):
        ...
        self.current_cases = self.current_cases[np.isin(
            self.current_cases['cell_line'], self.chosen_cell_lines)].reset_index(drop=True)
