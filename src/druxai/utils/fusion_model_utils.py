...
def set_optimizers(model: FusionModel, optimizer: str, learning_rate: float, weight_decay: float = 0) -> Optimizer:
    """
    Build optimizers for the model.

    Parameters
    ----------
        model (FusionModel): The interaction model to optimize.
...
def set_schedulers(scheduler: str, optimizer: Optimizer) -> _LRScheduler:
    """
...
def setup_training(
...
) -> Tuple[FusionModel, SGD, float, int]:
    """
    ...
    Returns
    -------
        Tuple[FusionModel, SGD, float, int]: Returns the model, two optimizers, the best validation loss,
    ...
    if not cfg.resume:
        ...
        optimizer = set_optimizers(model, cfg.optimizer, cfg.learning_rate)
        scheduler = set_schedulers(cfg.scheduler, optimizer)
    ...
    return model, optimizer, scheduler, loss_func, best_val_loss
