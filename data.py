...
class LRPSet(Dataset):
    def __init__(self, nsplits):
        self.splits = nsplits

        self.drug_embeddings = pd.read_csv('./results/embeddings.csv', index_col=0)
        self.drug_fingerprints = pd.read_csv('./results/ECFP.csv', index_col=0)

        self.molecular_data_raw = pd.read_csv('./results/rna_data.csv', index_col=0)
        relevant_genes = np.array(pd.read_csv('./results/relevant_genes.csv'))
        self.molecular_data = self.molecular_data_raw.iloc[:,np.isin(self.molecular_data_raw.columns, relevant_genes)]
        
        means, stds = self.molecular_data.mean(axis=0), self.molecular_data.std(axis=0)
        self.molecular_data = (self.molecular_data-means)/(stds+0.1)
            

        self.molecular_names = np.array(self.molecular_data.columns)
        pd.DataFrame({'names': self.molecular_names}).to_csv('./results/correct_gene_order.csv')
        
        self.chosen_drugs = ['POZIOTINIB', 'TRAMETINIB', 'COBIMETINIB', 'DACOMITINIB', 'PELITINIB', 'IBRUTINIB', 'IDASANUTLIN', 'VINCRISTINE', 'SELUMETINIB','CANERTINIB', 'OSIMERTINIB' 'UPROSERTIB', 'DASATINIB', 'OSIMERTINIB']

        self.cases = pd.read_csv('./results/prediction_targets.csv', index_col=0).reset_index(drop=True)

        self.CELLTYPE = np.array(self.cases['cell_line'].drop_duplicates().sort_values())

        self.cell_ids_test_lists, self.cell_lines_test_lists = self.generate_train_and_test_ids(self.molecular_data.index) 
        

        self.drug_features = self.drug_embeddings.shape[0] + self.drug_fingerprints.shape[1]   #+ self.drug_embeddings.shape[1]
        self.molecular_features  = self.molecular_data.shape[1]
        self.ncelltypes = self.CELLTYPE.shape[0]
        

        print(str(self.molecular_features) + ' molecular features, ' + str(self.ncelltypes) + ' celltypes, ' + str(self.drug_features) + ' drug_features')
    ...
    def __getitem__(self,idx):
        ...
        ##node2vec drug embedding
        current_drug_embedding = tc.tensor(self.drug_embeddings[current_drugname]).float().squeeze()
        current_drug_fingerprint = tc.tensor(self.drug_fingerprints.loc[current_drugname,:]).float() if current_drugname in self.drug_fingerprints.index else tc.zeros(self.drug_fingerprints.shape[1])

        #one hot drug embedding
        #current_drug_embedding_one_hot = tc.tensor((self.drug_embeddings.columns == current_drugname)*1.0).float().squeeze()

        current_drug_embedding_with_dose = tc.cat((current_drug_embedding, current_drug_fingerprint), axis=0)

        current_molecular_data = tc.tensor(self.current_molecular_data.loc[current_model_id,:]).float().squeeze()
